...
" Last Change:		Nov 08, 2013
" Version:		135	ASTRO-ONLY
...
syn cluster shCommandSubList	contains=shArithmetic,shComment,shDeref,shDerefSimple,shEcho,shEscape,shNumber,shOption,shPosnParm,shExSingleQuote,shSingleQuote,shExDoubleQuote,shDoubleQuote,shStatement,shVariable,shSubSh,shAlias,shTest,shCtrlSeq,shSpecial,shCmdParenRegion
...
syn region shTestDoubleQuote	contained	matchgroup=shQuote start='\%(\%(\\\\\)*\\\)\@<!"' skip=+\\"+ end='"' contains=shDerefSimple,shDeref,@Spell
...
syn match  shDerefVar	contained	"{\@<=\h\w*"		nextgroup=@shDerefVarList
syn match  shDerefVar	contained	"{\@<=[0-9]"		nextgroup=@shDerefVarList
...
" Useful ksh and bash Keywords: {{{1
" ====================
if exists("b:is_kornshell") || exists("b:is_bash")
...
" =====================
 if exists("b:is_bash")
  syn keyword shStatement bind builtin caller compopt dirs disown enable help local logout mapfile popd pushd readarray shopt source
 else
